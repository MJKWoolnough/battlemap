		...
		this.node = g([
			...
			this.extra = g({"transform": `translate(${this.x}, ${this.y})`, "style": "color: #000"}, [
				...
				this.hp = g({"style": currentHP === null || maxHP === null ? "display: none" : undefined}, [
					...
					this.hpBar = use({"href": "#5e-hp", "width": this.width / 4, "height": this.width / 4, "stroke-dasharray": `${Math.PI * 19 * 0.75 * (maxHP ?? 0) / (currentHP ?? 1)} 60`, "style": `color: rgba(${Math.round(255 * (currentHP ?? 0) / (maxHP || 1))}, 0, 0, 1)`}),
