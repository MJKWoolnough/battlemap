...
const defaultLanguage = {
	...
	"en-GB": defaultLanguage
      ...
      jsonReg = /^\/character\/([0-9]+)\/json$/,
      processID = (_id: string) => {};
...
addPlugin(lang["PLUGIN_NAME"], {
	"menuItem": {
		"priority": 0,
		"fn": [lang["PLUGIN_NAME"], div({"onpopout": (e: CustomEvent) => beyondWindow = e.detail, "onpopin": () => beyondWindow = null}, [
			...
			button({"onclick": () => {
				if (parseInt(beyondID.value) + "" === beyondID.value) {
					...
					const url = new URL(beyondID.value),
					      jsonMatch = url.pathname.match(jsonReg);
					if (jsonMatch && parseInt(jsonMatch[1]) + "" === jsonMatch[1]) {
						processID(jsonMatch[1]);
					} else {
