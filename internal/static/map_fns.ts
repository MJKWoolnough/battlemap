...
noop = <T>(e: T) => e,
makeLayerContext = (folder: SVGFolder, fn: (path: string) => void, disabled = "", path = "/"): List => (folder.children as SortNode<SVGFolder | SVGLayer>).filter(e => e.id >= 0).map(e => isSVGFolder(e) ? menu(e.name, makeLayerContext(e, fn, disabled, path + e.name + "/")) : (e.name === "disabled" ? disable : noop)(item(e.name, fn.bind(e, path + e.name))));
