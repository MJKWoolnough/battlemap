...
export default function (url: string): Promise<Readonly<RPCType>>{
	return RPC(url, 1.1).then(rpc => {
		return Object.freeze({
			...
			"waitCharacterDataChange":     () => rpc.await(-34, true),
			"waitCharacterDataChangeUser": () => rpc.await(-34, true).then(d => (d.data = userData(d.data), d)),
			"waitCharacterDataRemove":     () => rpc.await(-36, true),
			"waitMapChange":               () => rpc.await(-32, true),
			"waitLayerAdd":                () => rpc.await(-38, true),
			"waitLayerFolderAdd":          () => rpc.await(-39, true),
			"waitLayerMove":               () => rpc.await(-40, true),
			"waitLayerRename":             () => rpc.await(-41, true),
			"waitLayerRemove":             () => rpc.await(-42, true),
			"waitMapLightChange":          () => rpc.await(-43, true),
			"waitMapInitiative":           () => rpc.await(-44, true),
			"waitLayerShow":               () => rpc.await(-45, true),
			"waitLayerHide":               () => rpc.await(-46, true),
			"waitLayerMaskAdd":            () => rpc.await(-47, true),
			"waitLayerMaskChange":         () => rpc.await(-48, true),
			"waitLayerMaskRemove":         () => rpc.await(-49, true),
			"waitTokenAdd":                () => rpc.await(-50, true),
			"waitTokenRemove":             () => rpc.await(-51, true),
			"waitTokenMoveLayer":          () => rpc.await(-52, true),
			"waitTokenMovePos":            () => rpc.await(-53, true),
			"waitTokenSetToken":           () => rpc.await(-54, true),
			"waitTokenSetImage":           () => rpc.await(-55, true),
			"waitTokenSetPattern":         () => rpc.await(-56, true),
			"waitTokenChange":             () => rpc.await(-57, true),
			"waitTokenFlip":               () => rpc.await(-58, true),
			"waitTokenFlop":               () => rpc.await(-59, true),
			"waitTokenSnap":               () => rpc.await(-60, true),
			"waitTokenSourceChange":       () => rpc.await(-61, true),
