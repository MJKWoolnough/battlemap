...
export default function (url: string): Promise<Readonly<RPCType>>{
	return RPC(url, 1.1).then(rpc => {
		return Object.freeze({
			...
			"waitLayerFolderAdd":    () => rpc.await(-34, true),
			"waitLayerMove":         () => rpc.await(-35, true),
			"waitLayerRename":       () => rpc.await(-36, true),
			"waitLayerRemove":       () => rpc.await(-37, true),
			"waitMapLightChange":    () => rpc.await(-38, true),
			...
			"waitTokenMoveLayer":    () => rpc.await(-47, true),
			"waitTokenMovePos":      () => rpc.await(-48, true),
			"waitTokenSetToken":     () => rpc.await(-49, true),
			"waitTokenSetImage":     () => rpc.await(-50, true),
			"waitTokenSetPattern":   () => rpc.await(-51, true),
			"waitTokenChange":       () => rpc.await(-52, true),
			"waitTokenFlip":         () => rpc.await(-53, true),
			"waitTokenFlop":         () => rpc.await(-54, true),
			"waitTokenSnap":         () => rpc.await(-55, true),
			"waitTokenSourceChange": () => rpc.await(-56, true),
