	...
	const {createHTML, clearElement} = await include("html.js"),
		...
		const asset = function(a) {
			const details = function() {
				...
				createHTML(overlay.addLayer(), {"class": "assetDetails"}, [
					createHTML("h1", [
						...
						createHTML("span", "⌫", {"class": "delete", "onclick": function() {
							createHTML(overlay.addLayer(), {"class": "assetDelete"}, [
					...
					a.Type === "image" ? createHTML("img", {"src": `assets/${a.ID}.${a.Ext}`, "style": "max-width: 50%; max-height: 50%;"}) : a.Type === "audio" ? createHTML("audio", {"controls": "controls", "src": `assets/${asset.ID}.${asset.Ext}`}) : [],
					createHTML("h2", "Tags"),
					createHTML("select", {"onchange": function() {
					...
					createHTML("ul", allTags.filter(t => a.Tags.includes(t.id)).map(t => createTagLine(t)))
				]);
			      },
			      html = function() {
				return createHTML("li", {"class": "assetName"}, createHTML("span", {"onclick": details}, a.Name));
			      },
		...
		const tags = new Map(), tagNames = new Map(),
			...
			const s = createHTML("ul"),
			      h = t.ID === 0 ? s : createHTML("li", {"class": "tag"}, [
					createHTML("label", t.Name.split("/").pop(), {"for": "tag_" + bID}),
					t.ID > 0 ? [
						createHTML("span", "✍", {"class": "rename", "onclick": function() {
							const renamer = this;
							createHTML(overlay.addLayer(), {"class": "tagRename"}, [
								...
								createHTML("input", {"id": "tagRename", "type": "text", "value": t.Name, "onkeypress": enterKey}),
								createHTML("button", "Rename", {"onclick": function() {
