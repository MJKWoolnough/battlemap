...
mapView = (mapData: MapData, loadChars = false) => {
	...
	const definitions = globals.definitions,
		...
		return {
	      ...
	      root = globals.root = svg({"id": "map", "style": {"position": "absolute"}, width, height, "tabindex": -1}, [definitions[node], layerList[node], rect({"width": "100%", "height": "100%", "fill": `rgba(0, 0, 0, ${isAdmin ? 127 : 255})`, "mask": "url(#mapMask)"})]),
